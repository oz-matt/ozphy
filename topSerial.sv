...
`include "pcie_basic_gen2_serial_hdl_interconnect_sv_wrapper.sv"
`include "top_progSerial.sv"
module test_top;
//-----------------------------------------------------------------------
...
//-----------------------------------------------------------------------
	...
	pcie_basic_gen2_serial_if pcie_mac_if ();
	pcie_basic_gen2_serial_if pcie_phy_if ();
	pcie_basic_gen2_serial_hdl_interconnect_sv_wrapper hdl_interconnect(pcie_mac_if, pcie_phy_if);
	...
	initial begin
		...
		top_prog env ;
		env = new("PCI Express Env", pcie_mac_if.TxRx, pcie_phy_if.TxRx, pcie_phy_if.Monitor );
		env.start();
